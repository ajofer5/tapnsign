...
const BADGE_QR_Y = 10;
const BADGE_TEXT_Y = BADGE_QR_Y + BADGE_QR_SIZE + 22; // 252
const BADGE_H = BADGE_TEXT_Y + 12; // 264

let _badgeWasmReady = false;
async function ensureBadgeWasm() {
  if (!_badgeWasmReady) {
    await initWasm(fetch('https://esm.sh/@resvg/resvg-wasm@2.4.1/index_bg.wasm?target=deno'));
    _badgeWasmReady = true;
  }
}

async function generateVerifyBadge(certificateId: string, autographId: string, userId: string): Promise<string> {
  const verifyUrl = `https://ophinia.com/verify/${certificateId}`;

  // QR: white modules on black background
  const qrSvg: string = await QRCode.toString(verifyUrl, {
    type: 'svg',
    margin: 1,
    color: { dark: '#FFFFFF', light: '#000000' },
  });
  const qrBytes = new TextEncoder().encode(qrSvg);
  const qrBinary = String.fromCharCode(...qrBytes);
  const qrDataUri = `data:image/svg+xml;base64,${btoa(qrBinary)}`;

  const badgeSvg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
  width="${BADGE_W}" height="${BADGE_H}" viewBox="0 0 ${BADGE_W} ${BADGE_H}">
  <rect width="${BADGE_W}" height="${BADGE_H}" fill="#000000"/>
  <image href="${qrDataUri}" x="${BADGE_QR_X}" y="${BADGE_QR_Y}" width="${BADGE_QR_SIZE}" height="${BADGE_QR_SIZE}"/>
  <text x="${BADGE_W / 2}" y="${BADGE_TEXT_Y}" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" font-size="14" fill="#ffffff" opacity="0.7">ophinia.com</text>
</svg>`;

  await ensureBadgeWasm();
  const resvg = new Resvg(badgeSvg, { fitTo: { mode: 'width', value: BADGE_W } });
  const pngBuffer = resvg.render().asPng();

  const badgePath = `${userId}/verify_badges/${autographId}.png`;
  const { error: uploadError } = await supabaseAdmin.storage
    .from(BADGE_BUCKET)
    .upload(badgePath, pngBuffer, { contentType: 'image/png', upsert: true });

  if (uploadError) throw new Error(`Verify badge upload failed: ${uploadError.message}`);
...
Deno.serve((req) =>
  handleRequest(async (request) => {
    ...
    assert(profile.is_creator === true, 403, 'You must be 18 or older to create autographs.');
    ...
    // Guard: certificate_id must be populated. On databases where the column was added
    // without a DEFAULT (via `alter table ... add column if not exists certificate_id text`),
    // the DB returns null here. Fail-fast so the badge is never generated with a null ID.
    if (!autograph.certificate_id) {
      await removeUploadedAssets(uploadedPaths);
      await supabaseAdmin.from('autographs').delete().eq('id', autograph.id);
      throw new Error('certificate_id was not generated by the database. Run migration 20260601000300_fix_certificate_id_default.sql on the production database.');
    }
    ...
    // Generate verify badge — required; fail mint if this fails
    try {
      const verifyBadgeUrl = await generateVerifyBadge(autograph.certificate_id, autograph.id, user.id);
      const { error: badgeUpdateError } = await supabaseAdmin
        .from('autographs')
        .update({ verify_badge_url: verifyBadgeUrl })
        .eq('id', autograph.id);
    ...
    // Helper: full rollback including the uploaded badge
    const rollbackAll = async () => {
      await supabaseAdmin.storage.from(BADGE_BUCKET).remove([`${user.id}/verify_badges/${autograph.id}.png`]);
      await removeUploadedAssets(uploadedPaths);
      await supabaseAdmin.from('media_assets').delete().eq('autograph_id', autograph.id);
      await supabaseAdmin.from('autographs').delete().eq('id', autograph.id);
    };
