-- Auction settlement hardening.
-- Adds resumable settlement state plus transactional RPCs for auction lifecycle completion.

alter table public.autographs
  add column if not exists auction_settlement_status text not null default 'none',
  add column if not exists auction_settlement_bid_id uuid references public.bids(id) on delete set null,
  add column if not exists auction_settlement_at timestamptz;

alter table public.autographs
  drop constraint if exists autographs_auction_settlement_status_check;

alter table public.autographs
  add constraint autographs_auction_settlement_status_check
  check (auction_settlement_status in ('none', 'pending_capture', 'settled', 'unsold'));

create index if not exists autographs_auction_settlement_status_idx
  on public.autographs (auction_settlement_status, auction_ends_at);
...
begin
  ...
  return jsonb_build_object(
    ...
    'creator_id', v_autograph.celebrity_id,
    'losers', v_loser_records
  );
end;
$$;
