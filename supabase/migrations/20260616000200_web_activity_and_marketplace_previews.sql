...
-- App-facing marketplace and public listing RPCs now surface only print-enabled
-- autographs. Digital-only listings (sale_state = 'fixed', prints_enabled = false)
-- belong to the web marketplace and must not appear in the app.
...
returns table (
  ...
  video_url text,
  thumbnail_url text,
  print_layout_url text,
  print_preview_url text,
  ...
  creator_personalized_requests_enabled boolean,
...
as $$
  select
    ...
    creator.personalized_requests_enabled as creator_personalized_requests_enabled,
  ...
  where a.status = 'active'
    ...
    and (a.print_limit is null or coalesce(print_counts.total_print_count, 0) < a.print_limit)
    and (
    ...
    and not exists (
      ...
      where (bu.blocker_id = coalesce(p_viewer_id, auth.uid()) and bu.blocked_user_id = a.owner_id)
         ...
         or (bu.blocker_id = coalesce(p_viewer_id, auth.uid()) and bu.blocked_user_id = a.creator_id)
         or (bu.blocker_id = a.creator_id and bu.blocked_user_id = coalesce(p_viewer_id, auth.uid()))
    )
...
drop function if exists public.get_public_listing(uuid, uuid);
...
grant execute on function public.get_public_listing(uuid, uuid) to authenticated, anon;
