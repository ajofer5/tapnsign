-- Add print_layout_url cache to autographs table so the profile page can show
-- the assembled print layout (selfie + signature on grey) instead of the raw
-- video thumbnail. Populated lazily the first time a visitor opens Preview Print.

alter table public.autographs
  add column if not exists print_layout_url text;

comment on column public.autographs.print_layout_url is
  'Cached URL of the assembled print layout image, populated lazily by the web print-preview API.';

-- Update the profile page RPC to include print_layout_url in listing objects
