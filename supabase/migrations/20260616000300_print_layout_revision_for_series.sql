alter table public.autographs
  add column if not exists print_layout_revision integer not null default 1;

alter table public.autographs
  drop constraint if exists autographs_print_layout_revision_positive;

alter table public.autographs
  add constraint autographs_print_layout_revision_positive
  check (print_layout_revision > 0);

comment on column public.autographs.print_layout_revision is
  'Incremented whenever print-visible metadata changes so cached print layouts/previews use a new immutable Bunny path.';
...
begin
  ...
  set
    ...
    print_layout_revision = a.print_layout_revision + 1,
