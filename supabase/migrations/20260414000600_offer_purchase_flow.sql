-- Convert accepted autograph offers into payment-backed purchases.

alter type public.payment_purpose add value if not exists 'accepted_offer_purchase';

alter table public.autograph_offers
  add column if not exists accepted_at timestamptz,
  add column if not exists payment_due_at timestamptz,
  add column if not exists payment_event_id uuid references public.payment_events(id) on delete set null,
  add column if not exists accepted_transfer_id uuid references public.transfers(id) on delete set null;

create unique index if not exists autograph_offers_payment_event_unique_idx
  on public.autograph_offers (payment_event_id)
  where payment_event_id is not null;

create unique index if not exists autograph_offers_accepted_transfer_unique_idx
  on public.autograph_offers (accepted_transfer_id)
