...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!sessionId || !paymentEventId) {
    return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=missing`, request.url));
  }
  ...
  try {
    ...
    if (stripeSession.payment_status !== 'paid' || !stripeSession.payment_intent) {
      return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=unpaid`, request.url));
    }
    ...
    if (error || !result) {
      return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=finalize`, request.url));
    }

    return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?status=success`, request.url));
  } catch {
    return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=stripe`, request.url));
  }
}
