...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) {
    return NextResponse.redirect(new URL(`/login?next=${encodeURIComponent(`/offers/${offerId}/checkout`)}`, request.url));
  }
  ...
  if (
    ...
    return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=offer`, request.url));
  }

  if (await usersAreBlocked(user.id, offer.owner_id)) {
    return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=blocked`, request.url));
  }
  ...
  if (!paymentEventId) {
    ...
    if (existingEvent && existingEvent.amount_cents === offer.amount_cents && existingEvent.provider_metadata?.offer_id === offer.id) {
      ...
      if (insertError || !inserted) {
        return NextResponse.redirect(new URL(`/offers/${offerId}/checkout?error=payment`, request.url));
      }
  ...
  const successUrl = `${baseUrl}/offers/${offer.id}/checkout/success?session_id={CHECKOUT_SESSION_ID}&payment_event_id=${paymentEventId}`;
  const cancelUrl = `${baseUrl}/offers/${offer.id}/checkout?canceled=1`;
  ...
  try {
    ...
    if (!session.url) {
      return NextResponse.redirect(new URL(`/offers/${offer.id}/checkout?error=stripe`, request.url));
    }
