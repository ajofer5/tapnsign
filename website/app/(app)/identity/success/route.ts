...
export async function GET(request: NextRequest) {
  ...
  if (!sessionId || !paymentEventId) {
    return NextResponse.redirect(new URL('/identity?error=missing', request.url));
  }
  ...
  try {
    ...
    if (!isPaid) {
      return NextResponse.redirect(new URL('/identity?error=unpaid', request.url));
    }
    ...
    return NextResponse.redirect(new URL('/identity?status=success', request.url));
  } catch {
