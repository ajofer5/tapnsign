...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) {
    return NextResponse.redirect(new URL(`/login?next=${encodeURIComponent(`/checkout/${autographId}`)}`, request.url));
  }
  ...
  if (
    ...
    return NextResponse.redirect(new URL(`/checkout/${autographId}?error=listing`, request.url));
  }

  if (await usersAreBlocked(user.id, autograph.owner_id)) {
    return NextResponse.redirect(new URL(`/checkout/${autographId}?error=blocked`, request.url));
  }
  ...
  if (acceptedOfferLock) {
    return NextResponse.redirect(new URL(`/checkout/${autographId}?error=locked`, request.url));
  }
  ...
  if (existingEvent && existingEvent.amount_cents === autograph.price_cents) {
    ...
    if (insertError || !inserted) {
      return NextResponse.redirect(new URL(`/checkout/${autographId}?error=payment`, request.url));
    }
    paymentEventId = inserted.id;
  }

  const baseUrl = getWebsiteBaseUrl(request);
  const successUrl = `${baseUrl}/checkout/${autographId}/success?session_id={CHECKOUT_SESSION_ID}&payment_event_id=${paymentEventId}`;
  const cancelUrl = `${baseUrl}/checkout/${autographId}?canceled=1`;
  ...
  try {
    ...
    if (!session.url) {
      return NextResponse.redirect(new URL(`/checkout/${autographId}?error=stripe`, request.url));
    }
