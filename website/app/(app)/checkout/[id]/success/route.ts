...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!sessionId || !paymentEventId) {
    return NextResponse.redirect(new URL(`/checkout/${id}?error=missing`, request.url));
  }
  ...
  try {
    ...
    if (stripeSession.payment_status !== 'paid' || !stripeSession.payment_intent) {
      return NextResponse.redirect(new URL(`/checkout/${id}?error=unpaid`, request.url));
    }
    ...
    if (!paymentEvent || paymentEvent.user_id !== user.id || paymentEvent.autograph_id !== id) {
      return NextResponse.redirect(new URL(`/checkout/${id}?error=payment`, request.url));
    }
    ...
    if (error || !result) {
      return NextResponse.redirect(new URL(`/checkout/${id}?error=finalize`, request.url));
    }

    return NextResponse.redirect(new URL(`/checkout/${id}?status=success`, request.url));
  } catch {
    return NextResponse.redirect(new URL(`/checkout/${id}?error=stripe`, request.url));
  }
}
